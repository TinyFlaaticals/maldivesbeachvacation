...
var filter_controller_default = class extends Controller {
  ...
  submit() {
    ...
    if (facilityIds.length > 0) {
      facilityIds.forEach((id2) => params.append("facilities[]", id2));
    }
    if (activityIds.length > 0) {
      activityIds.forEach((id2) => params.append("activities[]", id2));
    }
    if (filterIds.length > 0) {
      filterIds.forEach((id2) => params.append("popular_filters[]", id2));
    }
    if (minPriceValue) {
